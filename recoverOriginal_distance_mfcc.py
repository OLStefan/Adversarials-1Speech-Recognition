                ...
                for target in targets:
                    ...
                    attacked_rate, attacked_wave = wav.read(
                        os.path.join("adversarials-" + attack_1 + "-" + attack_2, target,
                                     targets[target_1] + "_" + file))
                    attacked_mfcc_feat = python_speech_features.mfcc(attacked_wave, attacked_rate).flatten()
                    s.distance[targets.index(target)] = np.linalg.norm(np.subtract(orig_mfcc_feat, attacked_mfcc_feat))
